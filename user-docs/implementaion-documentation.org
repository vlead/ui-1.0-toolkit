...
extract the content of labs hosted in the Collaborative framework (Amrita UI format) to  UI 1.0 framework. 
...
a) After the lab developer has the content (theory, procedures, 
experiment, simulations, quiz, images, videos etc.) of the lab ready, the lab developer must understand the structure of the lab according to UI template 1.0.
...
c) While adding new experiments, the lab developer must create new directories following the same structure under ui/src/lab/"EXPERIMENT-ID" and change the content.html in the same directory. The lab developer should not edit the files index.html and default.html in the same directory. The ui/src/lab directory also contains js, images, and css directories. These directories hold the custom files of the lab developer. 	
#+BEGIN_SRC python :tangle a.py
ui/src/lib/ 	
#+END_SRC
...
d) The lab developer needs to choose the Blue-icon (uses the iitb style, which uses icons for sections) to design the theme of the lab. The current release also has a maroon-grid (which uses iiith style, which does not display icons). Of course, the developer is free to design his or her own theme, and place it in the themes directory.
...
The scripts folder should contain json file (lab descriptor), shell scripts, and dependencies file.
...
- *Initialize:* The *initialize* script creates the initial environment for the lab by installing the server side dependencies for the lab and invokes the *build* script. 
...
- *labspec.json:* This is an automated script for building the lab automatically on the container.
...
*Footer* contains the acknowledgement and licensing details of the proprietary software used. The lab developer may add the logo of the institute here. Header and Footer remains the same for all the webpages.
...
The command to do it is given in the following subsections:
