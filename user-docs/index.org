...
- *Unified look and feel:* The UI 1.0 Toolkit offers a uniform user
  ...
  also provides the labs a simple and uniform web interface and a
...
file =content.html= contains "simple pendulum" experiment by default.
...
** Convert Labs in Collaborative Framework (Amrita Format) to UI 1.0 Template
...
#+END_EXAMPLE
This command will get all the labs from all the disciplines of
IIT-KGP. Replace this url with your own institute's url.
...
If you get an error that resembles =index*\?sub*=, then open =organise.sh=, replace the line 
=sublist=$(ls -1 index*\?sub* | cut -d'&' -f1 | cut -d'=' -f2 | sort -u)=
with =sublist=$(ls -1 index*\_sub* | cut -d'&' -f1 | cut -d'=' -f2 | sort -u)=

*Step 7:* Copy the =kit= folder from
=../scripts/automated-scripts= and place inside each
experiment folder.

#+BEGIN_EXAMPLE :
rsync -avzr ../scripts/automated-scripts/kit ./1173/243/212/
#+END_EXAMPLE
...
folder. Content will open in the browser with Simple Pendulum sample
experiment.  You need to change the experiment name and add licensing
details and acknowledgements in the footer section.
