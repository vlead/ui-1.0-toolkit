...
Therefore, UI Toolkit 1.0 is a compilation of software tools used to build software programs that can run on the web. The toolkit is browser-compatible as it is based on Web Standards such as HTML5, CSS and JavaScript.The Vritual Labs UI 1.0 framework provides a set of CSS 
...
- *Build:* Build Script runs the *makefile* and copies the build folder to *localhost*. The script is invoked by *initialize* script.
